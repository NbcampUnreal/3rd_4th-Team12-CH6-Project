...
void UBonfireMenuLayoutWidget::HandleMeneUpAction()
{
	if (UWorld* World = GetWorld())
	{
		...
		{
			// 전송할 메시지 생성
			FBonfireMenuMoveMessage Message(0);

			// 메시지 브로드캐스트 (UI 전환용 채널로)
			MessageSubsystem->BroadcastMessage(TAG_Message_Channel_BonfireMenuMove, Message);
		}
	}
}
...
void UBonfireMenuLayoutWidget::HandleMenuDownAction()
{
	if (UWorld* World = GetWorld())
	{
		...
		{
			// 전송할 메시지 생성
			FBonfireMenuMoveMessage Message(1);
...
void UBonfireMenuLayoutWidget::HandleMenuSelectAction()
{
	if (UWorld* World = GetWorld())
	{
		...
		{
			// 전송할 메시지 생성
			FBonfireMenuMoveMessage Message(2);
